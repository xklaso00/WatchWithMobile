...
wordcount_t nativeNCount = uECC_curve_num_n_words(uECC_secp256k1());
wordcount_t nativeNCount20 = uECC_curve_num_n_words(uECC_secp160r1());
int byteLenght=28;
...
Java_cz_vutbr_feec_watchwithmobile_EccOperations_randPointWatch(JNIEnv *env, jobject thiz,jint SecLevel) {
    ...
    if(SecLevel==2) {
        ...
        randPoint2= reinterpret_cast<jbyte *>(new uECC_word_t[nativeNCount * 2]());
        ...
        byteLenght=20;
        nativeNCount = uECC_curve_num_n_words(uECC_secp160r1());
        uECC_generate_random_int(reinterpret_cast<uECC_word_t *>(rando20), curve->n, nativeNCount20); //generate random number max is n of curve
        const uECC_word_t* g = uECC_curve_G(curve);
        randPoint2= reinterpret_cast<jbyte *>(new uECC_word_t[nativeNCount20 * 2]());
        uECC_point_mult(reinterpret_cast<uECC_word_t *>(randPoint2), g,
                        reinterpret_cast<const uECC_word_t *>(rando20), curve);
    }

     //multiply G with random number, save to rando
...
extern "C"
JNIEXPORT jbyteArray  JNICALL
Java_cz_vutbr_feec_watchwithmobile_EccOperations_CforTk2Watch(JNIEnv *env, jobject /* this */,jbyteArray Tv, jint SecLevel) {
    if(SecLevel==0) {
        curve=uECC_secp160r1();
        byteLenght=20;
        nativeNCount = uECC_curve_num_n_words(uECC_secp160r1());
        jbyteArray cTv= reinterpret_cast<jbyteArray>(env->GetByteArrayElements(Tv, NULL));
        jbyte* point1= reinterpret_cast<jbyte *>(new uECC_word_t[nativeNCount20 * 2]());
        ...
        curve=uECC_secp224r1();
        byteLenght=28;
        nativeNCount = uECC_curve_num_n_words(uECC_secp256k1());
    } else{
...
jbyte * pub160= reinterpret_cast<jbyte *>(new uECC_word_t[nativeNCount20*2]());
extern "C"
JNIEXPORT jbyteArray  JNICALL
Java_cz_vutbr_feec_watchwithmobile_EccOperations_GenerateSecKey(JNIEnv *env, jobject /* this */,jint SecLevel) {
    ...
    else if(SecLevel==2)
    {
        ...
        uECC_generate_random_int(reinterpret_cast<uECC_word_t *>(randNum), uECC_secp160r1()->n, nativeNCount20);
        byteLenght=20;
        uECC_point_mult(reinterpret_cast<uECC_word_t *>(pub160), uECC_secp160r1()->G,
                        reinterpret_cast<const uECC_word_t *>(randNum), uECC_secp160r1());
    }
    jbyteArray newArray = env->NewByteArray(byteLenght);
...
Java_cz_vutbr_feec_watchwithmobile_EccOperations_GetPubKey(JNIEnv *env, jobject /* this */,jint SecLevel) {
    if (SecLevel==1)
    {
        ...
        byteLenght=20;
        jbyteArray newArray = env->NewByteArray(byteLenght*2);
        env->SetByteArrayRegion(newArray, 0,byteLenght*2, pub160);
        return newArray;
    }
